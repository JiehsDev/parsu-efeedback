...
// /login UI (src/app/(public)/login/page.tsx) for each of the five roles
// and persists the resulting session cookie via `storageState` so the rest
// of the suite never has to pay the cost of a real sign-in per test.
//
...
const ROLES: Array<{ role: UserRole; email: string; storageFile: string }> = [
  ...
  { role: "college_dean", email: "dean@parsu.edu.ph", storageFile: "tests/e2e/.auth/dean.json" },
